...
extension ARMeshGeometry {
    ...
    func vertex(at index: UInt32) -> SIMD3<Float> {
        let vertexPoint = vertices.buffer.contents().advanced(by: vertices.offset + (vertices.stride * Int(index)))
        let vertex = vertexPoint.assumingMemoryBound(to: SIMD3<Float>.self).pointee
        return vertex
    }
    

    func convertToMesh(device: MTLDevice, camera: ARCamera, modelMatrix: simd_float4x4) -> MDLMesh {
